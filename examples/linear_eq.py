...
if iterations == 1000:
    print(
        f"min_value: {objective.min()}, solution: {pop[objective.argmin()]}, generation: {iterations}"
    )
else:
    print(
        f"min_value: {objective[objective == 0][0]}, solution: {pop[objective == 0][0]}, generation: {iterations}"
    )
